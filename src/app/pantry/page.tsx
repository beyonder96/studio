...
import { Apple, Beef, Carrot, ChevronsRight, Fish, Milk, MoreHorizontal, Plus, Minus, Package, Soup, GlassWater, SparklesIcon, Search } from 'lucide-react';
...
export default function PantryPage() {
  ...
  const filteredItems = useMemo(() => {
    if (!searchTerm) return pantryItems;
                ...
                <div className="flex flex-col gap-6">
                  ...
                  {hasFilteredItems ? (
                      ...
                      <div className="text-center text-muted-foreground py-16 border-2 border-dashed rounded-lg">
                          ...
                          <h3 className="mt-4 text-lg font-medium">{hasItems ? 'Nenhum item encontrado' : 'Sua despensa está vazia'}</h3>
                          <p className="mt-1 text-sm">{hasItems ? 'Tente uma busca diferente.' : 'Adicione itens manualmente ou finalize uma lista de compras.'}</p>
                      </div>
                  )}
                </div>
            </CardContent>
